...
export class BuscadorComponent implements OnInit {
  ...
  constructor(private fb: FormBuilder, private route: Router, private searchService: BuscadorService) { }
  ngOnInit(): void {
    ...
    this.options$ = this.searchService.getOrigins().pipe(
      map((response: any) => {
        if (response && response.cities && q) {
          return response.cities.filter((city: any) =>
            city.name.toLowerCase().indexOf(q.toLowerCase()) > -1
          );
        }
        return [];
      })
